...
for i = breaki : floor(totaltime/steptime+0.01)-1
%display i
t00 = tic;
...
for  j=2:1:nnode-1  
 F(ndof*(j-1)+2,runningstep+1) = F_CL_FEM(i,j);  
 F(ndof*(j-1)+3,runningstep+1) = F_CD_FEM(i,j);  
end  
...
if i >= 3 && mod(steptime*(i+1),outputtime)~=0 
...
save('forcedata_con.mat','CL_all_rare','CD_all_rare','CL_all_true',...
    'CD_all_true','CL_all_FEM','CD_all_FEM','F_CL_FEM',...
    'F_CD_FEM','-v7.3')

save('displacement_con.mat','dis','vel','acc','-v7.3')
end
