function [outputArg1] = initialdvelxocity(inputArg1,N_strip,Vel_matrix)
%inputArg1 = 'fluids'
...
for j = 1:length(filename)
    ...
    file_old = [];
    p = 1;
    fid=fopen([name_temp,'/',filename_temp{num_zero},'/U']);
    while(feof(fid) ~= 1)
        line = fgetl(fid);
        file_old{p} = line;
        p = p+1;
    ...
    start = strfind(file_old,'internalField   uniform ');

    start_em = cellfun('isempty',start);
    num_cylinder = find(start_em==0);

    file_new = file_old;
    temp = ['internalField   uniform (',num2str(Vel_matrix(j)),32, ...
        num2str(0),32,'0);'];
    file_new{num_cylinder} = temp;

    fileID_new=fopen([name_temp,'/',filename_temp{num_zero},'/U'],'w+');
    [M,N]=size(file_new);
    for q=1:N
